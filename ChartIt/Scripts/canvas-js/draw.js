  ...
  //  blue-green colours) in both sentiment and topic tabs
{
  ...
  for( tab = 0; tab <= 5; tab++ ) {	// For all tabs with legends
    ...
    for( i = 0; i < 25; i++ ) {		// For 25 circles in legend
      r = 13 - ( i / 2 );		// Radius decreases along circles
      a = 1.0 - ( i / 22.0 * 0.9 );	// Alpha decreases along circles
      L = 0.56 + ( i / 25.0 * 0.3 );	// Luminance increases along circles

      if ( i < 7 ) {			// First 7 circles are green..
          col = get_colour_w_L(L, 0.55);
          ctx.fillStyle =
              "rgba( " + (col.r + 140) + "," + (col.g + 170) + "," + (col.b - 20) + "," + a + " )";
      } else {				// ..remaining circles are blue
          L = 0.676 + (i / 55.0 * 0.3);
          col = get_colour_w_L(L, 0.08);

          ctx.fillStyle =
              "rgba( " + (col.r + 55) + "," + (col.g - 30) + "," + (col.b +60) + "," + a + " )";
      }
    ...
    ctx.fillText("confident", w / 2, top + 15);
    ctx.fillText( "unpleasant", w / 2, top + 315 );
    ctx.fillText( "unconfident", w / 2, top + 330 );
  }					// End for all tabs
  ...
  //  tweet:  Tweet to inspect
{
  ...
  try {
    ...
    L = 0.56 + ( ( ( tweet.avg[ ARO ] - 1 ) / 8.0 ) * 0.3 );
    if (tweet.avg[VAL] < 5) {

        L = 0.676 + ( ( ( tweet.avg[ ARO ] - 1 ) / 8.0 ) * 0.3 );
